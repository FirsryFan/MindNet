...
| 字段 | 必填 | 规则 |
|---|---|---|
| `mark` | ✅ | `W / C / S / T / N / OK`（`OK` = 你写了 `✓`） |
| `text` | ✅ | **印刷正文为准**。`W`/`C` 若页边与正文不一致 ⇒ 用正文校正，并标 `corrected: true` |
| `margin_note` | `W`/`C`/`N` 必填 | 页边你写的那一份原样抄录（含拼写错误），用于核对 |
| `context` | `W`/`C` 必填 | 它在正文里的那一句（切到句号为止）—— 这就是例句的来源 |
| `page` / `line(s)` | ✅ | 行号从本页第 1 行起数；跨行给 `lines: [起, 止]` |
| `contains` | `S`/`T` 可选 | 指向**本批次内**的 `W`/`C` id（表示"这句里有这些词"） |
| `meaning` | 可选 | 中文释义；`meaning_source` 只能是 `ai`（下游会提示核对）或 `user`（你自己写的） |
| `uncertain` | 可选 | `true` = 有东西看不清；同时给 `region`（人话描述位置） |
| `conflict` | 可选 | `true` + `conflict_detail`：页边与正文冲突且无法判断 |
| `inferred_type` | 可选 | `true` = 你没写类型字母，是 AI 按长度猜的 |
| `duplicate_of` | 可选 | 同一批次内重复的条目指向先出现的那条 |

### 2.2 AI 不许做的事

1. **不许改你的意思**：你没标的东西不提取（哪怕它觉得那个词更重要）；
2. **不许补全没听清的内容**（`N` 类只记录你的疑问，不去替你猜答案）；
3. **不许编 `line`**：数不清行号就标 `uncertain`；
4. **不许把释义当事实**：`meaning` 一律 `meaning_source: "ai"`，卡片上会标"待核对"；
5. **不许合并**不同条目（重复的用 `duplicate_of` 显式表达）。
...
你现在会同时有两个"什么时候复习"的来源：Anki 自己的 SRS，和 MindNet 的
`memoryDsr.scheduleInterval`。**同时开两个会互相打架**（同一张卡，Anki 说明天、MindNet 说三天后）。
三种可行安排，选一个：

| 安排 | 谁排程 | MindNet 的角色 | 适合 |
|---|---|---|---|
| **A（推荐）** | **MindNet / Furnace** | 唯一排程者：出"什么时候碰这条线索"；Anki 只当渲染与音频播放器（可选导出） | 你主要在 Furnace 里学 |
| B | Anki | 只做诊断与建议；把 Anki 的 revlog 导出后回灌成 `review` 事件 | 你主要在 Anki 里学 |
| C | 两边都开 | ❌ 不要 | — |

本流水线默认按 **A** 生成（卡片不带 `due`）；如果你选 B，加 `--due-from-schedule` 会用
MindNet 的 `scheduleInterval` 给 Anki TSV 补一列到期时间，但**之后就别再用 MindNet 的排程提示**。
...
【记号表】
- 页边竖线 = 范围；竖线顶端的字母是类型：
  W 词（页边会照抄） / C 搭配（页边会照抄） / S 句子（不抄，正文即内容）
  T 语段（不抄，覆盖多行） / N 疑问（页边写了问题） / ✓ 我已知（不要建条目）
- 字母后面的修饰符：★ 高优先级 / → 要能产出 / ? 待确认 / ! 例外
- 页面顶部可能有 #主题 与日期

【硬规则】
1. 拼写以**印刷正文**为准；页边抄写只用于消歧与核对。两者不一致时用正文，
   并记下 margin_note 与 corrected: true。实在判不了就 conflict: true。
2. W/C/N 必须有 margin_note（页边原样，含拼写错误）；W/C 必须有 context（正文里的那一句）。
3. 行号从本页第 1 行开始数；数不清就标 uncertain: true 并描述位置，不要猜。
4. 句子/语段里的 W/C 用 contains 指出来（只能指向本批次内的 id）。
...
□ W/C/N 有 margin_note；W/C 有 context
